...
if(isset($_POST['envoi'])){ //si utilisateur appuie sur le bouton d'envoi
    if(!empty($_POST['pseudo']) AND !empty($_POST['mdp'])){ //si les champs ne sont pas vides
        ...
        $recupUser = $bdd->prepare('SELECT * FROM aministration WHERE login = ? AND mdp = ?'); //récupère l'utilisateur dans la table administration auquel correspond les champs donnés
